...
class VanillaRNN(object):
    ...
    def step_forward(self, x, prev_h):
        ...
        ############################################################################
        # Rubric - next = current input*input_weight + previous_state*hidden_weight 
        # + bias
        I = np.dot(x, self.params[self.wx_name])
        H = np.dot(prev_h, self.params[self.wh_name])
        B = self.params[self.b_name]
        #print(x.shape)
        #print(self.params[self.wx_name].shape)
        # tanh
        next_h = np.tanh(I+H+B)
    ...
    def step_backward(self, dnext_h, meta):
        ...
        dWx = np.dot(dAct.T, x).T
        dWh = np.dot(dAct.T, prev_h).T
    ...
    def forward(self, x, h0):
        ...
        ##############################################################################
        weight = np.random.rand(x.shape[0], x.shape[1], h0.shape[1])
        layer_args = self.step_forward(x[:,0,:], h0)
        self.meta.append(layer_args[1])
        weight[:,0,:] = layer_args[0]
        for i in range(1,x.shape[1]):
            layer_args = self.step_forward(x[:,i,:], weight[:,i-1,:])
            self.meta.append(layer_args[1])
            weight[:,i,:] = layer_args[0]
        h = weight
        pass
        ##############################################################################
        #                               END OF YOUR CODE                             #
    ...
    def backward(self, dh):
        ...
        ##############################################################################
        (N,T,H), D = dh.shape, self.meta[1][0].shape[1]
        dx = np.random.rand(N,T,D) 
        dh0 = np.random.rand(N,H)
        self.grads[self.wx_name] = np.zeros((D,H))
        self.grads[self.wh_name] = np.zeros((H,H))
        self.grads[self.b_name] = np.zeros((H,))
        last_weight = dh[:,T-1,:]
        for i in range(T-1,-1,-1):
            ret = self.step_backward(last_weight, self.meta[i])
            # return dx, dprev_h, dWx, dWh, db
            dx[:,i,:] = ret[0]
            if i:
                last_weight = ret[1] + dh[:,i-1,:]
            else:
                dh0 = ret[1]
            self.grads[self.b_name]  += ret[4]
            self.grads[self.wh_name] += ret[3]    
            self.grads[self.wx_name] += ret[2]
        pass
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
...
class LSTM(object):
    ...
    def step_forward(self, x, prev_h, prev_c):
        ...
        csh = prev_h.shape[1]
        # activation is R^4H
        act = np.dot(x, self.params[self.wx_name]) +\
         np.dot(prev_h, self.params[self.wh_name]) +\
         self.params[self.b_name]
        # gates
        input_gate = sigmoid(act[:,:csh])
        forget_gate = sigmoid(act[:,csh:2*csh])
        output_gate = sigmoid(act[:,2*csh:3*csh])
        gate_gate = np.tanh(act[:,3*csh:4*csh])
        # cell state c[t]
        next_c = forget_gate*prev_c + input_gate*gate_gate
        # hidden state h[t]
        next_h = output_gate*np.tanh(next_c)
        # wtv is needed for back prop
        meta = [
        [next_c, next_h, prev_c, prev_h],
        {'i':input_gate, 'f':forget_gate, 'o':output_gate, 'g':gate_gate},
        x
        ]
        pass
        #############################################################################
    ...
    def step_backward(self, dnext_h, dnext_c, meta):
        ...
        #############################################################################
        [[next_c, next_h, prev_c, prev_h], gates, x] = meta
        # update previous and next cell states
        dnext_c = dnext_c + np.multiply(gates['o']*dnext_h,(1-np.tanh(next_c)**2))
        dprev_c = gates['f']*dnext_c
        # get backprop of gates
        dG = np.multiply(gates['i'],dnext_c)
        dO = np.multiply(dnext_h,np.tanh(next_c))
        dF = np.multiply(prev_c,dnext_c)
        dI = np.multiply(gates['g'],dnext_c)
        # get backprop of activations after gates
        dGact = np.multiply(1-gates['g']**2, dG) 
        dOact = np.multiply((1-gates['o'])*gates['o'], dO)
        dFact = np.multiply((1-gates['f'])*gates['f'], dF)
        dIact = np.multiply((1-gates['i'])*gates['i'], dI)
        # concat 0h:1h, 1h:2h, 2h:3h, 3h:4h    
        dAct = np.concatenate((dIact, dFact, dOact, dGact), axis=1)
        # compute gradients mentioned above
        dx = np.dot(dAct, self.params[self.wx_name].T)
        dprev_h = np.dot(dAct, self.params[self.wh_name].T)
        dWh = np.dot(dAct.T, prev_h).T
        dWx = np.dot(dAct.T, x).T
    ...
    def forward(self, x, h0):
        ...
        prev_h, prev_c = h0, np.zeros((N,H)) # init state set to zero
        weight = np.random.rand(N,T,H)
        self.meta = [D]
        for i in range(T):
            next_h, next_c, tmp = self.step_forward(x[:,i,:], prev_h, prev_c)
            self.meta.append(tmp)
            weight[:, i, :], prev_h, prev_c = next_h, next_h, next_c
        # vanilla rnn
        h = weight
        pass
        #############################################################################
    ...
    def backward(self, dh):
        ...
        for i in range(T-1,-1,-1):
            dx[:,i,:], dprev_h, dprev_c, dWx_i, dWh_i, db_i =\
             self.step_backward(dh[:,i,:]+dprev_h, dprev_c, cache[i])
            dWx, dWh, db = dWx+dWx_i,dWh+dWh_i,db+db_i
        self.grads[self.wx_name], self.grads[self.wh_name], self.grads[self.b_name], dh0\
        =dWx, dWh, db, dprev_h
        pass
        #############################################################################
        #                               END OF YOUR CODE                            #
